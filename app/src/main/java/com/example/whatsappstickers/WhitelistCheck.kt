...
//    private const val STICKER_APP_AUTHORITY = BuildConfig.CONTENT_PROVIDER_AUTHORITY
    ...
    private fun isWhitelistedFromProvider(context: Context, identifier: String, whatsappPackageName: String): Boolean {
        ...
        if (isPackageInstalled(whatsappPackageName, packageManager)) {
            ...
            val queryUri = Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(whatsappProviderAuthority).appendPath(QUERY_PATH).appendQueryParameter(AUTHORITY_QUERY_PARAM, StickerPackLoader.authority).appendQueryParameter(IDENTIFIER_QUERY_PARAM, identifier).build()
