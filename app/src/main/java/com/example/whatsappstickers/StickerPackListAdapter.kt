...
class StickerPackListAdapter internal constructor(private var stickerPacks: List<StickerPack>, private val onAddButtonClickedListener: OnAddButtonClickedListener) : RecyclerView.Adapter<StickerPackListItemViewHolder>() {
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, i: Int): StickerPackListItemViewHolder {
        ...
        val stickerPackRow = layoutInflater.inflate(R.layout.sticker_packs_list_item, viewGroup, false)
    ...
    override fun onBindViewHolder(viewHolder: StickerPackListItemViewHolder, index: Int) {
        ...
        viewHolder.container.setOnClickListener { view: View ->
            ...
            intent.putExtra(StickerPackDetailsActivity.Companion.EXTRA_SHOW_UP_BUTTON, true)
            intent.putExtra(StickerPackDetailsActivity.Companion.EXTRA_STICKER_PACK_DATA, pack)
        ...
        for (i in 0 until actualNumberOfStickersToShow) {
            val rowImage = LayoutInflater.from(context).inflate(R.layout.sticker_packs_list_image_item, viewHolder.imageRowView, false) as SimpleDraweeView
            rowImage.setImageURI(StickerPackLoader.getStickerAssetUri(pack.identifier, pack.stickers!![i]!!.imageFileName))
        ...
        viewHolder.animatedStickerPackIndicator.visibility = if (pack.animatedStickerPack) View.VISIBLE else View.GONE
    }
    ...
    private fun setAddButtonAppearance(addButton: ImageView?, pack: StickerPack) {
        if (pack.isWhitelisted) {
            ...
            addButton.setOnClickListener { v: View? -> onAddButtonClickedListener.onAddButtonClicked(pack) }
            val outValue = TypedValue()
            addButton.context.theme.resolveAttribute(android.R.attr.selectableItemBackground, outValue, true)
