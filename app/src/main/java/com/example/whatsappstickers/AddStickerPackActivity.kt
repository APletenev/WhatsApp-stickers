...
abstract class AddStickerPackActivity : BaseActivity() {
    ...
    private fun createIntentToAddStickerPack(identifier: String, stickerPackName: String): Intent {
        ...
        intent.putExtra(StickerPackDetailsActivity.Companion.EXTRA_STICKER_PACK_ID, identifier)
        intent.putExtra(StickerPackDetailsActivity.Companion.EXTRA_STICKER_PACK_AUTHORITY, StickerPackLoader.authority)
        intent.putExtra(StickerPackDetailsActivity.Companion.EXTRA_STICKER_PACK_NAME, stickerPackName)
        return intent
    }

    fun onNewButtonClick() {
        val stickerPickerIntent = Intent(Intent.ACTION_OPEN_DOCUMENT_TREE)
        stickerPickerIntent.addCategory(Intent.CATEGORY_DEFAULT);
        startActivityForResult(stickerPickerIntent, PICKFILE_RESULT_CODE)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
                ...
                if (data != null) {
                    ...
                    if (validationError != null) {
                        if (BuildConfig.DEBUG) {
                            ...
                            MessageDialogFragment.Companion.newInstance(R.string.title_validation_error, validationError).show(supportFragmentManager, "validation error")
                        }
        ...
        when (requestCode) {
            PICKFILE_RESULT_CODE -> if (resultCode == RESULT_OK) {
                val uriFrom = data!!.data!!
                copyStickerToInternalStorage(uriFrom, applicationContext)
            }
        }
