...
internal object StickerPackValidator {
    ...
    fun verifyStickerPackValidity(context: Context, stickerPack: StickerPack) {
        ...
        check(!(!TextUtils.isEmpty(stickerPack.androidPlayStoreLink) && !isValidWebsiteUrl(stickerPack.androidPlayStoreLink))) { "Make sure to include http or https in url links, android play store link is not a valid url: " + stickerPack.androidPlayStoreLink }
        check(!(!TextUtils.isEmpty(stickerPack.androidPlayStoreLink) && !isURLInCorrectDomain(stickerPack.androidPlayStoreLink, PLAY_STORE_DOMAIN))) { "android play store link should use play store domain: $PLAY_STORE_DOMAIN" }
        ...
        check(!(!TextUtils.isEmpty(stickerPack.iosAppStoreLink) && !isURLInCorrectDomain(stickerPack.iosAppStoreLink, APPLE_STORE_DOMAIN))) { "iOS app store link should use app store domain: $APPLE_STORE_DOMAIN" }
        check(!(!TextUtils.isEmpty(stickerPack.licenseAgreementWebsite) && !isValidWebsiteUrl(stickerPack.licenseAgreementWebsite))) { "Make sure to include http or https in url links, license agreement link is not a valid url: " + stickerPack.licenseAgreementWebsite }
        check(!(!TextUtils.isEmpty(stickerPack.privacyPolicyWebsite) && !isValidWebsiteUrl(stickerPack.privacyPolicyWebsite))) { "Make sure to include http or https in url links, privacy policy link is not a valid url: " + stickerPack.privacyPolicyWebsite }
        ...
        check(!(!TextUtils.isEmpty(stickerPack.publisherEmail) && !Patterns.EMAIL_ADDRESS.matcher(stickerPack.publisherEmail).matches())) { "publisher email does not seem valid, email is: " + stickerPack.publisherEmail }
        try {
            val stickerAssetBytes = StickerPackLoader.fetchStickerAsset(stickerPack.identifier, stickerPack.trayImageFile!!, context.contentResolver)
    ...
    @Throws(IllegalStateException::class)
    private fun validateSticker(context: Context, identifier: String, sticker: Sticker, animatedStickerPack: Boolean) {
    ...
    @Throws(IllegalStateException::class)
    private fun validateStickerFile(context: Context, identifier: String, fileName: String, animatedStickerPack: Boolean) {
        try {
            val stickerInBytes = StickerPackLoader.fetchStickerAsset(identifier, fileName, context.contentResolver)
            ...
            try {
                ...
                if (animatedStickerPack) {
                    ...
                    checkFrameDurationsForAnimatedSticker(webPImage.frameDurations, identifier, fileName)
    ...
    private fun checkFrameDurationsForAnimatedSticker(frameDurations: IntArray, identifier: String, fileName: String) {
